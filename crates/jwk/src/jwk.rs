...
impl fmt::Display for JWKError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            JWKError::SerializationError { message } => write!(f, "Serialization error: {}", message),
        }
    }
}
...
fn compute_thumbprint(jwk: &JWK) -> Result<String, JWKError> {
    ...
    let bytes = serde_json::to_vec(&thumbprint_payload)
        .map_err(|err| JWKError::SerializationError { message: format!("Failed to serialize thumbprint payload: {}", err) })?;
