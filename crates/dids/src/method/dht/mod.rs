...
use crate::{bearer::BearerDid, document::{Document, Service, VerificationMethod}, identifier::Identifier, resolver::{ResolutionMetadata, ResolutionResult}};

use self::{bep44::encode_bep44_message, convert::document_packet::document_to_packet, registered_types::RegisteredDidType};
  ...
  fn create(
    ...
    if options.publish {
      let packet = document_to_packet(&document).map_err(|e| {
        MethodError::DidCreationFailure(format!("Failed to convert did document to DNS packet: {}", e))
      })?;

      let packet_bytes: Vec<u8> = packet.build_bytes_vec_compressed().map_err(|_| -> MethodError {
        MethodError::DidCreationFailure("Failed to serialize DNS packet to bytes".to_string())
      })?.into();
      ...
      let response = client.put(DEFAULT_GATEWAY_URL)
          .header("Content-Type", "application/octet-stream")
          .body(bep44_message) 
          .send()
          .map_err(|e| {
            MethodError::DidCreationFailure(
              format!("Failed to publish did {} to gateway {}: {}", uri, DEFAULT_GATEWAY_URL, e)
            )
          })?;
