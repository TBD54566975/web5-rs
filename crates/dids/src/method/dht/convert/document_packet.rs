...
pub fn document_to_packet(document: &Document) -> Result<Packet<'static>, ConvertError> {
    ...
    document.verification_method.iter().enumerate().try_for_each(
      |(idx, vm)| -> Result<(), ConvertError>{
        let idx = idx as u32;
      ...
      if let Some(service) = document.service {
        service.iter().enumerate().try_for_each(|(idx, src)| -> Result<(), ConvertError> {
          let idx = idx as u32;
      ...
      packet.answers.push(
        root_record.to_txt_record(&document.id)?
      );
