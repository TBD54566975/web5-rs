    ...
    F: Fn(Vec<u8>) -> Result<Vec<u8>, KeyManagerError>,
{
    let microseconds_since_epoch = SystemTime::now()
        ...
        .map_err(|_| MethodError::DidCreationFailure("Could not get time since epoch".to_string()))?
        .as_micros();
    ...
    let signature = sign(signable)
        .map_err(|_| MethodError::DidCreationFailure("Failed to sign BEP44 message".to_string()))?;
