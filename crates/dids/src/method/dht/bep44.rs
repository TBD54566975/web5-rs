  ...
  F: Fn(Vec<u8>) -> Result<Vec<u8>, KeyManagerError>
{
  let microseconds_since_epoch = SystemTime::now()
    .duration_since(UNIX_EPOCH).map_err(|_| {
      MethodError::DidCreationFailure("Could not get time since epoch".to_string())
    })?
    .as_micros();

  let mut signable = format!("3:seqi{}e1:v{}:", microseconds_since_epoch, message.len()).into_bytes();
  signable.extend(message);

  let signature = sign(signable).map_err(|_| {
    MethodError::DidCreationFailure("Failed to sign BEP44 message".to_string())
  })?;

  let mut seq_bytes = vec![];
  seq_bytes.write_u128::<BigEndian>(microseconds_since_epoch);
