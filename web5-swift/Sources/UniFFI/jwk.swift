...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_jwk_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_jwk_rustbuffer_free(self, $0) }
    }
}
...
public class Jwk:
    ...
    public func uniffiClonePointer() -> UnsafeMutableRawPointer {
        return try! rustCall { uniffi_jwk_fn_clone_jwk(self.pointer, $0) }
    }
    public convenience init()  {
        self.init(unsafeFromRawPointer: try! rustCall() {
    uniffi_jwk_fn_constructor_jwk_new($0)
})
    }

    deinit {
        try! rustCall { uniffi_jwk_fn_free_jwk(pointer, $0) }
    }
    ...
    public func computeThumbprint()  -> String {
        return try!  FfiConverterString.lift(
            try! 
    rustCall() {
    
    uniffi_jwk_fn_method_jwk_compute_thumbprint(self.uniffiClonePointer(), $0
    )
}
        )
    }
...
private var initializationResult: InitializationResult {
    ...
    let scaffolding_contract_version = ffi_jwk_uniffi_contract_version()
    if bindings_contract_version != scaffolding_contract_version {
        return InitializationResult.contractVersionMismatch
    }
    if (uniffi_jwk_checksum_method_jwk_compute_thumbprint() != 43621) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_jwk_checksum_constructor_jwk_new() != 35568) {
        return InitializationResult.apiChecksumMismatch
    }
